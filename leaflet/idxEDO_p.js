...
function cargaSeleccionEDO(){
	mapRegionEDO.removeLayer(geojsonmunicipioRegEdo);
	geojsonmunicipioRegEdo =L.geoJson(municipios, {
	        ...
	        filter: function(feature, layer) {
				        if (feature.properties.CVE_ENT === EDOvigente){
				        	return true;
				        };
				    },
		zoomToFeature
...
function generalinks_otro(leyenda,datos){
...
function preparaEstado(clave){
    ...
    var tableSLC = $('#TabEscalaEDO').DataTable();
	tableSLC.destroy();
    ...
    $("#ER_sel").append(muestraEnPlot(datEstado[0][2],datRegiones,"R"));
...
function vistaregRegEdo(clavebuscada){
	...
	procesoRegEdo= procesoRegEdo +1
		...
		if (n===2 && Number(clavebuscada) > 0 && Number(clavebuscada) < 3300 ){
			if ( procesoRegEdo >1 ){mapRegionEDO.removeLayer(geojsonRegEdo);};
			geojsonRegEdo =L.geoJson(regiones, {
