...
function cargaSeleccionMunicipal(){
	 mMPOvigente= MPOvigente
	 //var mMPOvigente= window.location.href.substr(window.location.href.length -5);
     var mEDOvigente= EDOvigente; 
     var mREGvigente= REGvigente;
     var mMesoREGvigente= MesoREGvigente;

    mapMunicipioMPO.removeLayer(geojsonmunicipioMpoMesoReg); 
    mapMunicipioMPO.removeLayer(geojsonmunicipioMpoEnt); 
    mapMunicipioMPO.removeLayer(geojsonmunicipioMpoReg); 
...
function initMapMunicipal() {
	...
	geojsonmunicipioMpoMesoReg =L.geoJson(mesoregiones, {
	        ...
	        filter: function(feature, layer) {
				        if (feature.properties.MESOREGI_1 ==='01'){
				        	return true;
				        };
				    },
	...
	geojsonmunicipioMpoReg =L.geoJson(regiones, {
	        ...
	        filter: function(feature, layer) {
				        if (feature.properties.ENTREG === '0101'){
				        	return true;
				        };
				    },
	...
	geojsonmunicipioMpoEnt =L.geoJson(entidades, {
	        ...
	        filter: function(feature, layer) {
				        if (feature.properties.CVE_ENT === '01'){
				        	return true;
				        };
				    }
...
function generalinks_OtroPP(leyenda,datos){
...
function preparaMunicipio(clave){
	$('#seccionMunicipios').removeClass('hidden');
	tooglehome('seccionMunicipios');

	console.log(clave);
	cargaSeleccionMunicipal();
	//inicializando
    ...
    var tableSLC = $('#TabEscalaMPO').DataTable();
	tableSLC.destroy();
...
function vistaregMpoReg2(clavebuscada){
	...
	procesoMpoReg2= procesoMpoReg2 +1
		...
		if (n===4 && Number(clavebuscada) > 0 && Number(clavebuscada) < 33000 ){
			if ( procesoMpoReg2 >1 ){mapMunicipioREG.removeLayer(geojsonMpoReg);};
			...
			}).addTo(mapMunicipioREG2);
			//zoomToFeature;
			mapMunicipioREG2.fitBounds(geojsonMpoReg.getBounds());
		};
